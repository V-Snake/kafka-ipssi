...
# -*- coding: utf-8 -*-
"""
Exercice 8 - Visualisation simp                        except json.JSONDecodeError as e:
                            print(f"[WARN] Ligne JSON invalide dans {alerts_file}:{line_num}")
                        
        except Exception as e:
            print(f"[ERROR] Erreur lors de la lecture de {alerts_file} : {e}")
    
    print(f"[OK] Total de {len(records)} records charges")des logs météo

Version simplifiée pour visualiser rapidement les données météo stockées localement.

Usage:
    python simple_weather_viz.py ./hdfs-final-test
"""
...
# Vérification des imports avec fallback
try:
    ...
    print("⚠️  matplotlib non disponible. Installation: pip install matplotlib")
...
try:
    ...
    HAS_PANDAS = False
    print("⚠️  pandas non disponible. Installation: pip install pandas")
            ...
            with open(alerts_file, 'r', encoding='utf-8') as f:
                for line_num, line in enumerate(f, 1):
                    ...
                    try:
                        ...
                        print(f"⚠️  Ligne JSON invalide dans {alerts_file}:{line_num}")
                        
        except Exception as e:
            print(f"❌ Erreur lors de la lecture de {alerts_file} : {e}")
    
    print(f"✅ Total de {len(records)} records chargés")
    return records

def analyze_data_basic(records: List[Dict]) -> Dict:
    """Analyse basique des données sans pandas"""
    if not records:
        return {}
...
def print_analysis_report(analysis: Dict):
    ...
    print("\n" + "="*60)
    print("📊 RAPPORT D'ANALYSE - DONNÉES MÉTÉOROLOGIQUES")
    print("="*60)
    ...
    if analysis['temperatures']:
        ...
        print("🌡️  STATISTIQUES DE TEMPÉRATURE (°C) :")
    ...
    if analysis['weather_codes']:
        print("☁️  CODES MÉTÉO LES PLUS FRÉQUENTS :")
...
def create_simple_plots(records: List[Dict], analysis: Dict, output_dir: str = "./visualizations"):
    ...
    if not HAS_MATPLOTLIB:
        print("[WARN] matplotlib non disponible pour creer les graphiques")
        return
    ...
    if analysis['temperatures']:
        ...
        plt.title("Distribution des températures", fontsize=14, fontweight='bold')
        ...
        plt.close()
        print(f"[OK] Graphique sauvegarde : {output_path}/temperature_distribution.png")
    
    # 2. Graphique des vitesses de vent
    if analysis['wind_speeds']:
        plt.figure(figsize=(10, 6))
        ...
        plt.close()
        print(f"[OK] Graphique sauvegarde : {output_path}/wind_speed_distribution.png")
    
    # 3. Graphique des alertes
    alert_data = []
    alert_labels = []
    ...
    if alert_data:
        ...
        plt.title("Distribution des alertes météo", fontsize=14, fontweight='bold')
        ...
        plt.close()
        print(f"[OK] Graphique sauvegarde : {output_path}/alerts_distribution.png")
    
    # 4. Graphique des codes météo par pays
    if analysis['countries'] and analysis['weather_codes']:
        # Trouve le code météo le plus fréquent globalement
        ...
        plt.close()
        print(f"[OK] Graphique sauvegarde : {output_path}/countries_distribution.png")

def main():
    parser = argparse.ArgumentParser(description="Visualisation simple des logs météo")
    parser.add_argument("data_path", help="Répertoire contenant les logs (ex: ./hdfs-final-test)")
    parser.add_argument("--limit", type=int, help="Limiter le nombre de records")
    parser.add_argument("--output-dir", default="./visualizations", help="Répertoire de sortie")
    parser.add_argument("--no-plots", action="store_true", help="Ne pas créer de graphiques")
    
    args = parser.parse_args()
    
    try:
        # Charge les données
        print("[START] Demarrage de l'analyse des donnees meteo")
        ...
        # Analyse les données
        print("[ANALYZE] Analyse des donnees...")
        ...
        if not args.no_plots:
            print("[PLOTS] Creation des graphiques...")
            create_simple_plots(records, analysis, args.output_dir)
        
        print("[SUCCESS] Analyse terminee avec succes !")
        return 0
